...
async function handleSubmit() {
  ...
  if (isEdit.value) { await api.put(`/honors/${currentRow.value.id}`, form.value); ElMessage.success('更新成功'); dialogVisible.value = false; savedId.value = null }
  else { const { data } = await api.post('/honors', form.value); savedId.value = data.id; isEdit.value = true; currentRow.value = data; ElMessage.success('创建成功，请上传证明材料') }
  loadData()
}

async function handleDelete(row) {
  await ElMessageBox.confirm('确定删除？', '提示', { type: 'warning' })
  await api.delete(`/honors/${row.id}`); ElMessage.success('删除成功'); loadData()
}
