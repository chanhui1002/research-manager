...
async function handleSubmit() {
  ...
  if (isEdit.value) { await api.put(`/awards/${currentRow.value.id}`, form.value); ElMessage.success('更新成功') }
  else { const { data } = await api.post('/awards', form.value); savedId.value = data.id; ElMessage.success('创建成功，请上传证明材料') }
  loadData()
}
